        ...
        with tempfile.TemporaryDirectory(prefix="threatlens-replay-") as log_dir:
            ...
            if shutil.which("docker") and has_docker_sock and not is_testing:
                try:
                    ...
                    logger.debug("Container docker execution omitted (%s). Running native PCAP parser.", docker_exc)

            # Strategy 2: If docker was not available, extract real packets directly into Zeek logs via PcapZeekEngine
            if not zeek_success:
                try:
                    from ingest.pcap_engine import PcapZeekEngine
                    engine = PcapZeekEngine(str(pcap_path))
                    engine.export_zeek_logs(log_dir)
                    logger.info("Exported real Zeek logs from %s into %s", pcap_path.name, log_dir)
