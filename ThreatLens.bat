...
echo.
echo  ===================================================================
echo   _______ _hre__    _____ ___                    
echo  ^|__   __^|  __ \  / ____/ _ \                  
echo     ^| ^|  ^| ^|__) ^|^| (___^| ^| ^| ^| ___ _ __  ___ 
echo     ^| ^|  ^|  _  /  \___ \^| ^| ^| ^|/ _ \ '_ \/ __^|
echo     ^| ^|  ^| ^| \ \  ____) ^| ^|_^| ^|  __/ ^| ^| \__ \
echo     ^|_^|  ^|_^|  \_\^|_____/\___/\___^|_^| ^|_^|___/
echo.
...
:: 3. Launch Docker Compose containers (Redpanda, Redis, ClickHouse, Zeek)
...
:: 4. Launch FastAPI Backend in a dedicated enclave window
echo [*] Starting ThreatLens Backend API & Streaming Gateway (Port 8000)...
start "ThreatLens Backend API" cmd /k "title ThreatLens Backend Service && set INGEST_SOURCE=kafka&& .\.venv\Scripts\uvicorn backend.app.main:app --port 8000 --reload"

echo [*] Initializing Kafka Consumers and Neural Pipeline...
ping 127.0.0.1 -n 5 >nul

:: 5. Ship initial Zeek PCAP logs to Redpanda/Kafka
...
:: 6. Launch React Frontend in a dedicated window
echo [*] Launching SOC Enclave Web Interface (Port 5173)...
start "ThreatLens Dashboard UI" cmd /k "title ThreatLens Frontend Console && cd frontend && npm run dev"

:: 7. Launch Browser to Frontend Dashboard
...
echo.
echo  ===================================================================
echo   ✓ ThreatLens SOC Enclave application started successfully!
